...
void testHotData_HashLFU() {
    ...
    for (int k = 0; k < HOT_KEYS; ++k) {
        hashLfu.put(k, "v" + std::to_string(k));
    }
    ...
    for (int op = 0; op < OPERATIONS; ++op) {
        ...
        if (isPut) {
            hashLfu.put(key, "val" + std::to_string(key));
        } else {
            std::string result;
            get_ops++;
...
// 场景3：工作负载剧烈变化
void testWorkloadShift_HashLFU() {
    auto timeStart = std::chrono::steady_clock::now(); // 计算起始时间
    std::cout << "\n=== 工作负载剧烈变化 ===" << std::endl;
