...
void printResults(const std::string& testName, int capacity, 
    ...
    for (size_t i = 0; i < hits.size(); ++i) {
        ...
        std::cout << "HashLfuCache - 命中率: " << std::fixed << std::setprecision(2) 
                  << hitRate << "% ";
...
void testHotDataAccess() {
    ...
    HashLfuCache<int, std::string> hashLfu(CAPACITY, 4); // 4分片示例
    ...
    for (int key = 0; key < HOT_KEYS; ++key) {
        hashLfu.put(key, "value" + std::to_string(key));
    }
    ...
    for (int op = 0; op < OPERATIONS; ++op) {
        ...
        if (isPut) {
            hashLfu.put(key, "value" + std::to_string(key) + "_v" + std::to_string(op % 100));
        } else {
            std::string result;
            get_ops++;
            if (hashLfu.get(key, result)) hits++;
        }
    }
...
void testLoopPattern() {
    ...
    HashLfuCache<int, std::string> hashLfu(CAPACITY, 4);
    std::random_device rd;
    std::mt19937 gen(rd());
    ...
    for (int key = 0; key < LOOP_SIZE/5; ++key) {
        hashLfu.put(key, "loop" + std::to_string(key));
    }
    ...
    for (int op = 0; op < OPERATIONS; ++op) {
        ...
        if (isPut) {
            hashLfu.put(key, "loop" + std::to_string(key) + "_v" + std::to_string(op % 100));
        } else {
            std::string result;
            get_ops++;
            if (hashLfu.get(key, result)) hits++;
...
void testWorkloadShift() {
    ...
    for (int key = 0; key < 30; ++key) {
        hashLfu.put(key, "init" + std::to_string(key));
    }
    ...
    for (int op = 0; op < OPERATIONS; ++op) {
        ...
        if (isPut) hashLfu.put(key, "value" + std::to_string(key) + "_p" + std::to_string(phase));
        else {
            std::string result;
            get_ops++;
            if (hashLfu.get(key, result)) hits++;
