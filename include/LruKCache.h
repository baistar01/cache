...
public:
    ...
    ,historyList_(unique_ptr<LruCache<Key, size_t>>(new LruCache<Key, size_t>(historyCapacity)))  // 设置为独占，当LruKCache销毁时也会被销毁
    ,k_(k) {}
...
private:
    int k_;
    unique_ptr<LruCache<Key, size_t>> historyList_;  // 访问数据的历史记录
    unordered_map<Key, Value> historyValueMap_;     // 存储未达到k次访问的数据值。
};
